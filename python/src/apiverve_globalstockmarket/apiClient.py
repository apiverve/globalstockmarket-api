...
class GlobalstockmarketAPIClientError(Exception):
...
class ValidationError(GlobalstockmarketAPIClientError):
...
class GlobalstockmarketAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"country": {"type": "string", "required": True, "minLength": 2, "maxLength": 2}, "year": {"type": "integer", "required": False, "min": 2000, "max": 2030}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the GlobalstockmarketAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises GlobalstockmarketAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/globalstockmarket'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise GlobalstockmarketAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[GlobalstockmarketAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns GlobalStockMarketData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises GlobalstockmarketAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise GlobalstockmarketAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise GlobalstockmarketAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise GlobalstockmarketAPIClientError(f"Request failed: {str(e)}")
